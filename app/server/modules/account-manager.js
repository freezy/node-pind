...
//            db2.run('DROP TABLE IF EXISTS users;');
			db.run('CREATE TABLE IF NOT EXISTS users (user VARCHAR PRIMARY KEY NOT NULL UNIQUE, pass TEXT NOT NULL, name VARCHAR, email VARCHAR, added DATETIME, credits INTEGER DEFAULT 30);');
		});
	}
});
...
exports.autoLogin = function(user, pass, callback) {
	db.get(findUserByName, [ user ], function(e, r) {
		if (r) {
...
exports.addNewAccount = function(newData, callback) {
	db.get(findUserByName, [ newData.user ], function(e, r) {
		if (r) {
			callback('username-taken');
		} else {
			saltAndHash(newData.pass, function(hash) {
				db.prepare('INSERT INTO users (user, pass, name, email, added) VALUES (?, ?, ?, ?, ?);')
...
exports.updateAccount = function(newData, callback) {
	db.get(findUserByName, [ newData.user ], function(e, r) {
		...
		if (newData.pass == '') {
			db.prepare('UPDATE users SET name = (?), email = (?) WHERE user = (?);')
				.run([ r.name, r.email, r.user ], function() {
					callback(null, r);
				});
		} else {
			saltAndHash(newData.pass, function(hash) {
				db.prepare('UPDATE users SET pass = (?), name = (?), email = (?) WHERE user = (?);')
...
exports.updatePassword = function(user, newPass, callback) {
	db.get(findUserByName, [ user ], function(e, r) {
		saltAndHash(newPass, function(hash) {
			db.prepare('UPDATE users SET hash = (?) WHERE user = (?);')
...
exports.deleteAccount = function(user, callback) {
	db.prepare('DELETE FROM users WHERE user = (?);').run([ user ], callback);
};
exports.validateResetLink = function(email, passHash, callback) {
	db.get('SELECT * FROM users WHERE user = (?) AND pass = (?);', [ newData.user, passHash ], function(e, r) {
		callback(o ? 'ok' : null);
	});
};
