...
function Hiscore(app) {
	if ((this instanceof Hiscore) === false) {
		return new Hiscore(app);
	}
	events.EventEmitter.call(this);
	this.initAnnounce(app);
}
util.inherits(Hiscore, events.EventEmitter);

/**
 * Sets up event listener for realtime updates via Socket.IO.
 * @param app Express application
 */
Hiscore.prototype.initAnnounce = function(app) {
	var an = require('./announce')(app, this);

	// fetchHighscores()
	an.data('processingStarted', { id: '#fetchhs' });
	an.data('processingCompleted', { id: '#fetchhs' });
	an.notice('processingCompleted', 'High scores successfully synchronized.', 5000);
	an.notice('processingFailed', 'Error synchronizing: {{err}}', 3600000);

	// matchHiscore()
	an.notice('readingStarted', 'Reading high scores from "{{rom}}" ({{name}})', 5000);

	// watchHischores()
	an.notice('nvramChangeDetected', 'Updated NVRAM, updating high score for ROM {{rom}}', 60000);
 ...
 * @param next Callback
 */
Hiscore.prototype._updateHighscore = function(hiscore, next) {
 ...
 * @param next Callback
 */
Hiscore.prototype._matchHiscore = function(tables, users, rom, next) {
	...
	that.getHighscore(rom, function(err, hiscore) {
		if (err || !hiscore) {
			...
			that.emit('readingStarted', { rom: rom, name: tables[rom].name });

			// now we have all high scores for this table, update them in the db
			async.eachSeries(hiscores, that._updateHighscore, next);
		}
	});
};
 ...
 */
Hiscore.prototype.assertAll = function(startWith) {
	if (!startWith) {
		startWith = '0';
	}
	var that = this;
	...
	async.eachSeries(nvrams, function(nvram, callback) {
		that.getHighscore(nvram, function(err, result) {
			if (!err) {
				if (result != null) {
					delete result.raw;
					console.log('\n' + nvram);
