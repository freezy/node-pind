...
/**
 * Notes freezy:
 *   In the registered methods we need access to the user object, which is stored in the request
 *   object. This version passes the request object as first parameter into each call.
 */
(function() {
	...
	var JRPCServer = function() {
		...
		var _dispatchInternal = function(req, res, jsonRequest) {
			...
			var _next = function() {
				...
				return _dispatchSingle(req, jReq, function(err, result) {
		...
		var _dispatchSingle = function(req, jReq, callbackFn) {
			...
			var parameters = [ req, jReq.params ];
			try {
		...
		this.paths = {
			'/version' : function(req, res) {
				return _this.output(res, '1.0.3');
			}
		};
		this.interceptor = function(jReq, next) {
			next();
		};
